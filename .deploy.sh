...
# Create namespace first (secrets need it to exist)
echo -e "${BLUE}Creating namespace...${NC}"
kubectl apply -f k8s/namespace.yaml

# Apply secrets manually (they are gitignored for security)
echo -e "${BLUE}Applying secrets (gitignored files)...${NC}"
kubectl apply -f k8s/secrets.yaml
kubectl apply -f k8s/duckdns-secret.yaml

# Apply Traefik manually (it goes to kube-system, not managed by ArgoCD)
echo -e "${BLUE}Applying Traefik Ingress Controller...${NC}"
